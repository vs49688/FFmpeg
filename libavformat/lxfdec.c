...
static int get_packet_header(AVFormatContext *s)
{
    ...
    if (version > 1)
        av_log_request_sample(s, "Format version %"PRIu32, version);
    ...
    case 1:
        ...
        if (st->codecpar->bits_per_coded_sample != (audio_format & 0x3F)) {
            av_log_report_missing_feature(s, "Not tightly packed PCM");
        ...
        default:
            av_log_report_missing_feature(s, "PCM not 16-, 20-, 24- or 32-bits");
            return AVERROR_PATCHWELCOME;
        }
