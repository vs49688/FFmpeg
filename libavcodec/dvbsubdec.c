...
static int save_subtitle_set(AVCodecContext *avctx, AVSubtitle *sub, int *got_output)
{
    ...
    if (sub->num_rects) {
        av_log_request_sample(ctx, "Different Version of Segment asked Twice");
                                       ...
                                       const uint8_t *buf, int buf_size)
{
    ...
    if (coding_method == 0) {
        ...
        av_log_report_missing_feature(avctx, "coded as a string of characters");
        return AVERROR_PATCHWELCOME;
    } else if (coding_method == 2) {
        av_log_report_missing_feature(avctx, "progressive coding of pixels");
        return AVERROR_PATCHWELCOME;
    } else {
